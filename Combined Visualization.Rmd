...
ccl_line <- st_read("~/Documents/DowntownLineAnalysis/shp/CCL Rail Line.shp")
nel_line <- st_read("~/Documents/DowntownLineAnalysis/shp/NEL Rail Line.shp")
...
```{r, echo = FALSE, results="hide", warning=FALSE, message=FALSE}
subzones <- st_read('~/Documents/DowntownLineAnalysis/shp/Planning_Area_Census2010.shp')
buffers <- st_read("~/Documents/DowntownLineAnalysis/shp/CCLNEL Multiring Buffers.shp") %>%
  mutate(unique = paste0(OBJECTID, ".", distance))
#priceShapes <- st_read("~/Documents/DowntownLineAnalysis/shp/CCLNEL Intersection.shp") %>%
#  mutate(unique = paste0(OBJECTID, ".", distance)) %>%
#  mutate(psm = resale_p_1 / floor_area) %>%
...
renderUI({
  plotOutput("unsized2", height = 1080)
})
```
